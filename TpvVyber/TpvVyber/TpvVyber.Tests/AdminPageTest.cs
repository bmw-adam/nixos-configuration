...
[NonParallelizable]
[TestFixture]
public class AdminPageTestIndex : EeTestClass
{
    [Test]
    public async Task TestAdminPageIndex()
    {
        ...
        await Page.GetByRole(AriaRole.Button, new PageGetByRoleOptions { Name = "delete-btn" })
            .ClickAsync();
        await Page.WaitForConsoleMessageAsync(
            new PageWaitForConsoleMessageOptions { Predicate = a => a.Text == "Finished deleting!" }
        );
        ...
        await Page.GetByRole(AriaRole.Button, new PageGetByRoleOptions { Name = "add-course-btn" })
            .ClickAsync();
        await Page.WaitForConsoleMessageAsync(
            new PageWaitForConsoleMessageOptions { Predicate = a => a.Text == "Finished Adding Course!" }
        );
        ...
        await Page.WaitForConsoleMessageAsync(
            new PageWaitForConsoleMessageOptions { Predicate = a => a.Text == "Finished Adding Student!" }
        );
