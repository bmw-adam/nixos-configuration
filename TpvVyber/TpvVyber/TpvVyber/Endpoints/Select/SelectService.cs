...
) : ISelectService
{
    ...
    )
    {
        ...
        // TODO finish searchnig
        var student = ctx.Students.SingleOrDefault(s => s.Email == userEmail);

        if (student == null)
        {
            throw new NullReferenceException("Nepodařilo se najít žáka/nepodařilo se přidat žáka");
        }

        // načti ordering studenta
        var ordering = await ctx.OrderCourses.Where(o => o.StudentId == student.Id).ToListAsync();
        ...
        // kurzy bez orderingu
        foreach (var course in availableCourses.Where(c => !ordering.Any(o => o.CourseId == c.Id)))
        {
            result[index++] = course.ToClient(ctx, fillExtended);
        }
    ...
    public async Task UpdateOrderAsync(Dictionary<int, CourseCln> input)
    {
        ...
        {
            throw new NullReferenceException("Nenašel jsem aktuálního uživatele.");
        }

        ctx.OrderCourses.RemoveRange(ctx.OrderCourses.Where(a => a.StudentId == actualStudent.Id));
